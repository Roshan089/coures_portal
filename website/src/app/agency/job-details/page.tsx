...
const JobDetails = () => {
  ...
  useEffect(() => {
    ...
    if (!applicantData) {
      router.push(AGENCY_ROUTES.JOB_APPLICATION);
    }
  }, [applicantData, router, setBreadcrumbs]);
  if (!applicantData) {
    return null;
  }
  const {data: jobDetails} = useGetJobByIdQuery(jobId as string);
            ...
            <Grid container justifyContent="space-between">
              ...
              >
                <AppIconButton
                  ...
                  onClick={() =>
                    router.push(
                      `${AGENCY_ROUTES.CANDIDATE_MESSAGING}?receiverId=${applicantData.applicant.userId}`
                    )
                  }
                >
                  Message Applicant
