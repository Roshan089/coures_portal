...
export function AdminApplicantGraph() {
  const theme = useTheme();

  const { data: applicantGraphData, isLoading } =
    useGetCompanyGraphQuery("applicant");
  const chartData =
    applicantGraphData?.planWiseRevenue.map((item) => ({
      ...
      >
        <Box>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            ...
            >
              {(applicantGraphData?.totalAmount || 0).toLocaleString("en-IN", {
